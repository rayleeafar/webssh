...
| Existing database migration to the new `credentials` table fails on legacy schemas before migration completes because the startup schema creates an index on `nodes.credential_id` before that column exists | 3 | AC-6 | Split legacy detection/migration from fresh schema creation, rebuild `nodes` with the final columns and constraints, and add a regression test that initializes from the pre-credentials schema |
